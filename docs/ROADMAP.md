...
- The shared editable-table component must provide its add-row action inside the table: render a final add column with an accessible `+` control for populated tables and an in-table empty-state add control when there are no rows. Do not place a separate add button beneath the table.
