import { Alert, Button, Card, Popconfirm, Spin } from "antd";
...
export function SupplierCatalogueEditPage() {
  ...
  return <FormPageLayout actions={<Popconfirm cancelText={t("purchasing.cancel")} description={t("purchasing.catalogue.confirmStatusDescription", { action: action.toLocaleLowerCase(i18n.language) })} okText={action} onConfirm={() => setStatus.mutateAsync(!item.isActive)} title={t("purchasing.catalogue.confirmStatusTitle", { action })}><Button danger={item.isActive} loading={setStatus.isPending}>{action}</Button></Popconfirm>} backLabel={t("purchasing.catalogue.title")} backTo={returnTo} title={t("purchasing.catalogue.editTitle")}>{update.error ? <Alert className="page-alert" message={getErrorMessage(t, update.error, "purchasing.catalogue.errors.update")} showIcon type="error" /> : null}{setStatus.error ? <Alert className="page-alert" message={getErrorMessage(t, setStatus.error, "purchasing.catalogue.errors.status")} showIcon type="error" /> : null}<Card><SupplierProductForm cancelLabel={t("purchasing.cancel")} errorMessageKey="purchasing.catalogue.errors.update" initialValues={{ supplierId: item.supplierId, productId: item.productId, supplierSku: item.supplierSku ?? undefined, purchaseUnitOfMeasure: item.purchaseUnitOfMeasure, minimumOrderQuantity: item.minimumOrderQuantity, unitPrice: item.unitPrice, currencyCode: item.currencyCode }} isEditing isSubmitting={update.isPending} onCancel={goBack} onSubmit={submit} submitLabel={t("purchasing.save")} /></Card></FormPageLayout>;
}
