...
export function PurchaseOrderEditPage() {
  ...
  const { goBack, returnTo } = useReturnDestination(purchasingRoutes.orderDetail(id ?? ""));
  ...
  return <FormPageLayout backLabel={t("purchasing.orders.detailTitle")} backTo={returnTo} title={t("purchasing.orders.editTitle")}>{update.error ? <Alert className="page-alert" message={getErrorMessage(t, update.error, "purchasing.orders.errors.update")} showIcon type="error" /> : null}<Card><PurchaseOrderForm cancelLabel={t("purchasing.cancel")} errorMessageKey="purchasing.orders.errors.update" initialValues={{ supplierId: order.supplierId, destinationWarehouseId: order.destinationWarehouseId, currencyCode: order.currencyCode, orderDate: order.orderDate, expectedDeliveryDate: order.expectedDeliveryDate, supplierReference: order.supplierReference, notes: order.notes, version: order.version, lines: order.lines.map(line => ({ supplierProductId: line.supplierProductId, quantity: line.quantity })) }} isSubmitting={update.isPending} onCancel={goBack} onSubmit={submit} submitLabel={t("purchasing.save")} /></Card></FormPageLayout>;
}
