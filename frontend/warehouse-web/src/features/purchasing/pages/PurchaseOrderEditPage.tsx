...
export function PurchaseOrderEditPage() {
  ...
  return <section><Typography.Title level={2}>{t("purchasing.orders.editTitle")}</Typography.Title>{update.error ? <Alert className="page-alert" message={getErrorMessage(t, update.error, "purchasing.orders.errors.update")} showIcon type="error" /> : null}<Card><PurchaseOrderForm errorMessageKey="purchasing.orders.errors.update" initialValues={{ supplierId: order.supplierId, lines: order.lines.map(line => ({ supplierProductId: line.supplierProductId, quantity: line.quantity })) }} isSubmitting={update.isPending} onSubmit={submit} submitLabel={t("purchasing.save")} /></Card></section>;
}
