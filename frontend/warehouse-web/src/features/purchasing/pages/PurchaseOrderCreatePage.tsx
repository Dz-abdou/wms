...
export function PurchaseOrderCreatePage() {
  ...
  return <section><Typography.Title level={2}>{t("purchasing.orders.createTitle")}</Typography.Title>{create.error ? <Alert className="page-alert" message={getErrorMessage(t, create.error, "purchasing.orders.errors.create")} showIcon type="error" /> : null}<Card><PurchaseOrderForm cancelLabel={t("purchasing.cancel")} errorMessageKey="purchasing.orders.errors.create" isSubmitting={create.isPending} onCancel={() => navigate(purchasingRoutes.orders)} onSubmit={submit} submitLabel={t("purchasing.orders.create")} /></Card></section>;
}
