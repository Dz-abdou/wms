...
export function SupplierCatalogueCreatePage() {
  ...
  return <section><Typography.Title level={2}>{t("purchasing.catalogue.createTitle")}</Typography.Title>{create.error ? <Alert className="page-alert" message={getErrorMessage(t, create.error, "purchasing.catalogue.errors.create")} showIcon type="error" /> : null}<Card><SupplierProductForm errorMessageKey="purchasing.catalogue.errors.create" isSubmitting={create.isPending} onSubmit={submit} submitLabel={t("purchasing.catalogue.create")} /></Card></section>;
}
