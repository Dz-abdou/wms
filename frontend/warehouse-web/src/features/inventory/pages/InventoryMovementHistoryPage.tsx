import { Alert, Button, Card, Empty, Form, Input, Select, Spin, Table } from "antd";
...
export function InventoryMovementHistoryPage() {
  const { i18n, t } = useTranslation(); const pagination = useListPagination(); const [filters, setFilters] = useState<MovementFilters>({});
  ...
  const columns: ColumnsType<InventoryMovement> = [
    ...
    { title: t("inventory.table.reference"), key: "reference", render: (_, x) => x.inventoryAdjustmentId ? <ReturnAwareLink to={inventoryRoutes.adjustmentDetail(x.inventoryAdjustmentId)}>{x.adjustmentReference ?? t("inventory.adjustment")}</ReturnAwareLink> : "—" },
