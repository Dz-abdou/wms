...
public sealed record SalesOrderInput(Guid CustomerId, Guid ShippingAddressId, Guid FulfillmentWarehouseId, string? CurrencyCode, DateOnly OrderDate, DateOnly? RequestedShipDate, string? CustomerReference, string? DeliveryInstructions, int? Version, IReadOnlyList<SalesOrderLineInput>? Lines);
...
public sealed record SalesOrderAvailabilityQuery(Guid FulfillmentWarehouseId, Guid[] ProductIds);
public sealed record SalesOrderAvailabilityResponse(Guid ProductId, string BaseUnitOfMeasure, decimal AvailableQuantityInBase);
