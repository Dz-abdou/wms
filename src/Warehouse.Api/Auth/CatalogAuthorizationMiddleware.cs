...
public sealed class CatalogAuthorizationMiddleware(RequestDelegate next)
{
    ...
    private static string? GetPolicy(HttpRequest request) => request.Path.StartsWithSegments("/api/admin") ? AuthorizationPolicies.ManageAdministration : request.Path.StartsWithSegments("/api/products") || request.Path.StartsWithSegments("/api/warehouses") ? request.Method == HttpMethods.Get ? AuthorizationPolicies.ReadCatalog : AuthorizationPolicies.ManageCatalog : null;
}
